...
RNAseq_data_make<-function(CancerSite,directories,lg=F,dtype='RNAseq',
...
#'/home/giordano/Trabalho/2019-2022/Doutorado/Development/ws-modmap/modmap/RNAseq_PreProcess/Resources/PP_RNA.R'

Preprocess_RNAseq <- function(CancerSite, directories,lg=F, mvt = 0.2,
  ...
  if (!is.na(directories)) {
    cat(paste0('\tLoading RNAseq data for ',CancerSite,'.\n'))
    cm=paste0('Rscript ',svtmp,'Trabalho/2019-2022/Doutorado/Development/ws-modmap/modmap_02/PreProcess/Script/aux_scripts/PP_RNA.R')
    system(cm)
    cat(paste0('\tFinished preprocessing RNAseq data for ',CancerSite,'.\n'))
    svd=paste0(svtmp,'ProcessedDataRNA.Rdata')
    load(svd)
  }else{
  ...
  PreProcessed_Data=list('ProcessedData'= ProcessedDataRNA)
